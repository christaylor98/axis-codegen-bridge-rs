use axis_codegen_bridge::runtime::value::{Value, intern_str, intern_tag, get_str, get_tag_name, init_runtime};
use axis_codegen_bridge::runtime::{arith, str_ops, list, option, bool_ops, transitions};
...
fn var_node(name: Value) -> Value {
    Value::Ctor { tag: intern_tag("var"), fields: vec![name] }
}
fn lam_node(param: Value, body: Value) -> Value {
    Value::Ctor { tag: intern_tag("lam"), fields: vec![param, body] }
}

#[test]
fn test_introduce_let_binding_returns_ctor() {
    setup();
    let r = transitions::introduce_let_binding(
        Value::Tuple(vec![Value::Int(1), Value::Int(5), Value::Int(99)])
    );
    match r {
        Value::Ctor { ref fields, .. } => {
            assert_eq!(fields[0], Value::Int(1));
            assert_eq!(fields[1], Value::Int(5));
            assert_eq!(fields[2], Value::Int(99));
        }
        _ => panic!("expected Ctor"),
    }
    assert_eq!(format!("{}", r), "let(1, 5, 99)");
}

#[test]
fn test_introduce_lambda_returns_ctor() {
    setup();
    let r = transitions::introduce_lambda(
        Value::Tuple(vec![Value::Int(2), Value::Int(42)])
    );
    assert_eq!(format!("{}", r), "lam(2, 42)");
    match &r { Value::Ctor { tag, .. } => assert_eq!(get_tag_name(*tag), "lam"), _ => panic!() }
}

#[test]
fn test_apply_function_returns_app_ctor() {
    setup();
    let r = transitions::apply_function(
        Value::Tuple(vec![Value::Int(3), Value::Int(7)])
    );
    assert_eq!(format!("{}", r), "app(3, 7)");
    match &r { Value::Ctor { tag, .. } => assert_eq!(get_tag_name(*tag), "app"), _ => panic!() }
}

#[test]
fn test_extract_subterm_to_function_returns_lam() {
    setup();
    let r = transitions::extract_subterm_to_function(
        Value::Tuple(vec![Value::Int(4), Value::Int(8)])
    );
    assert_eq!(format!("{}", r), "lam(4, 8)");
    match &r { Value::Ctor { tag, .. } => assert_eq!(get_tag_name(*tag), "lam"), _ => panic!() }
}

#[test]
fn test_inline_let_binding_substitutes_var() {
    setup();
    // body = Ctor("var", [Int(1)]) — var reference matching name Int(1)
    let body = var_node(Value::Int(1));
    let r = transitions::inline_let_binding(
        Value::Tuple(vec![Value::Int(1), Value::Int(5), body])
    );
    assert_eq!(r, Value::Int(5));
}

#[test]
fn test_inline_let_binding_no_match_unchanged() {
    setup();
    // body references Int(99), but binding name is Int(1) — no substitution
    let body = var_node(Value::Int(99));
    let r = transitions::inline_let_binding(
        Value::Tuple(vec![Value::Int(1), Value::Int(5), body.clone()])
    );
    assert_eq!(r, body);
}

#[test]
fn test_inline_let_binding_nested_substitution() {
    setup();
    // body = Ctor("app", [var(1), var(1)]) — two occurrences, both substituted
    let body = Value::Ctor {
        tag: intern_tag("app"),
        fields: vec![var_node(Value::Int(1)), var_node(Value::Int(1))],
    };
    let r = transitions::inline_let_binding(
        Value::Tuple(vec![Value::Int(1), Value::Int(42), body])
    );
    assert_eq!(r, Value::Ctor {
        tag: intern_tag("app"),
        fields: vec![Value::Int(42), Value::Int(42)],
    });
}

#[test]
fn test_rename_bound_variable_renames_param_and_body() {
    setup();
    // lam(1, var(1)) renamed 1→2 → lam(2, 2)
    let lam = lam_node(Value::Int(1), var_node(Value::Int(1)));
    let r = transitions::rename_bound_variable(
        Value::Tuple(vec![Value::Int(1), Value::Int(2), lam])
    );
    assert_eq!(format!("{}", r), "lam(2, 2)");
}

#[test]
fn test_rename_bound_variable_non_lam_passthrough() {
    setup();
    // If the third arg is not a "lam" Ctor, return it unchanged
    let not_a_lam = Value::Int(99);
    let r = transitions::rename_bound_variable(
        Value::Tuple(vec![Value::Int(1), Value::Int(2), not_a_lam])
    );
    assert_eq!(r, Value::Int(99));
}

#[test]
fn test_reference_registry_function_wraps_in_var_ctor() {
    setup();
    let r = transitions::reference_registry_function(Value::Int(42));
    assert_eq!(format!("{}", r), "var(42)");
    match &r {
        Value::Ctor { tag, fields } => {
            assert_eq!(get_tag_name(*tag), "var");
...
#[test]
fn test_verify_foreign_reference_always_true() {
    setup();
    assert_eq!(transitions::verify_foreign_reference(Value::Int(0)),  Value::Bool(true));
    assert_eq!(transitions::verify_foreign_reference(Value::Unit),    Value::Bool(true));
    assert_eq!(transitions::verify_foreign_reference(Value::Bool(false)), Value::Bool(true));
}
